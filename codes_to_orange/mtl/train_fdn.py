...
from modeling.fdn import FDN, FrobeniusLoss
import torch
...
def model_train(num_cam, epochs, model_dict, mmoe, fdn, alpha, train_dataloader, optimizer, scheduler, save_path, num_classes_dict, device, optim_type):
    for epoch in range(epochs):
        ...
        total_epoch_loss_dl_b2 = 0
        total_epoch_loss_dl_b3_12 = 0
        total_epoch_loss_dl_b3_13 = 0
        total_epoch_loss_dl_b3_23 = 0
        total_epoch_loss = 0
        ...
        for i, datas in enumerate(tqdm(train_dataloader)):
            ...
            losses_dict = {}
            dl_b2 = floss(features_b2[0], features_b2[1])
            dl_b3_12 = floss(features_b3[0], features_b3[1])
            dl_b3_13 = floss(features_b3[0], features_b3[2])
            dl_b3_23 = floss(features_b3[1], features_b3[2])
            ...
            for camid in camid_list:
                ...
                losses_dict[camid] = losses + alpha * (dl_b2 + dl_b3_12 + dl_b3_13 + dl_b3_23)
                total_epoch_loss_dl_b2 += alpha * dl_b2
                total_epoch_loss_dl_b3_12 += alpha * dl_b3_12
                total_epoch_loss_dl_b3_13 += alpha * dl_b3_13
                total_epoch_loss_dl_b3_23 += alpha * dl_b3_23
        ...
        logging.info('loss dl b2: {}'.format(total_epoch_loss_dl_b2))
        logging.info('loss dl b3 12: {}'.format(total_epoch_loss_dl_b3_12))
        logging.info('loss dl b3 13: {}'.format(total_epoch_loss_dl_b3_13))
        logging.info('loss dl b3 23: {}'.format(total_epoch_loss_dl_b3_23))
...
if __name__ == "__main__":
    ...
    if dataname == 'wild':
        ...
        n_cam = 6
    filename = os.path.join('mtl/logs/', dataname, mmoe_name, f'fdn_e{epochs}_lr{learning_rate}_alpha{alpha}_seed{seed}_optim{optim_type}')
    ...
    save_path = os.path.join(mmoe_path, f'fdn_e{epochs}_lr{learning_rate}_alpha{alpha}_seed{seed}_optim{optim_type}')
