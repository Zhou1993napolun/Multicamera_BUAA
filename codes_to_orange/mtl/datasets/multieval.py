...
class MultiEval(object):
    def __init__(self, root):
        ...
        self.gallery_mmoe_path = 'gallery_mmoe'
    ...
    def preprocess(self, subset='query', relabel=False):
        ...
        for sub_dir in sub_dirs:
            fpaths = sorted(glob(osp.join(sub_dir, '*.jpg')))
            # 图片目录，提取pid和camid
            if subset == 'gallery':
                sub_pids = {}
            tmp = []
            for fpath in fpaths:
                ...
                if subset in ['query', 'train', 'gallery_mmoe']:
                    if relabel:
    ...
    def load(self):
        self.query, self.query_ids = self.preprocess(self.query_path)
        self.gallery_mmoe, self.gallery_mmoe_ids = self.preprocess(self.gallery_mmoe_path)
        self.gallery, self.gallery_ids = self.preprocess(self.gallery_path)
