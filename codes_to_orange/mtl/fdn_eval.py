...
if True:
    ...
    from mtl.modeling.fdn import FDN
...
if __name__ == "__main__":
    ...
    fdnname = 'fdn_e100_lr0.001_alpha0_seed1121'
    mmoe_epoch = 60
    fdn_epoch = 10
    if len(sys.argv) > 1:
        fdn_epoch = int(sys.argv[1])
    if dataname == 'wild':
        n_cam = 7
        mmoe_path = 'mtl/saves/wild/' + mmoename
        fdn_path = os.path.join(mmoe_path, fdnname)
        log_path = 'mtl/logs/wild/' + mmoename
        ...
        fdn_path = os.path.join(mmoe_path, fdnname)
        log_path = 'mtl/logs/multi/' + mmoename
        log_path = os.path.join(log_path, fdnname)
    ...
    with open(os.path.join(log_path, f'eval_group_results_epoch{fdn_epoch}.txt'), 'w') as f:
        f.write(f'mAP: {mAP}\n')
        f.write(f'mINP: {mINP}\n')
