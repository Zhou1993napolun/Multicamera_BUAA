...
def model_train(num_cam, epochs, model_dict, fcs, train_dataloader, optimizers, save_path, num_classes_dict, device):
    for epoch in range(epochs):
        ...
        for i, datas in enumerate(tqdm(train_dataloader)):
            ...
            feas_total = {}
            for i in range(8):
                feas_total[i] = torch.zeros(num_cam, 256).to(device)
...
if __name__ == "__main__":
    # 设置随机数种子
    seed = 1121  # 你可以选择任何种子值, 测试的时候设置了111
    ...
    if dataname == 'wild':
        ...
        n_cam = 6
    filename = 'mtl/logs/' + dataname + '/reranked_seed' + str(seed) + '_epoch' + str(epochs) + '_relu' + '_hl' + str(nums_hl)
    ...
    for i in range(8):
        fcs[i] = FC(n_cam, nums_hl, 256).to(device)
    paras = {}
    ...
    save_path = os.path.join('mtl/saves', dataname, 'reranked_seed' + str(seed) + '_epoch' + str(epochs) + '_relu' + '_hl' + str(nums_hl))
