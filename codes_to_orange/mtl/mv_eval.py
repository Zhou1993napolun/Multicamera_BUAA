...
def process_gallery(model_dict, gallery_dataloader_dict):
...
def load_models(dataset, model_type='mgn_R50-ibn', is_origin=0):
    if dataset == 'wild':
        views = 7
        config_paths = {}
...
if __name__ == "__main__":
    ...
    is_origin_model = 0
    #######################################################################################
    eval_mode = 1  # 仅在orange_demo启用，设置为1
    name_g = '0119'  # 仅在orange_demo下有意义
    name_q = '0119'
    ##################################################################################
    if dataset == 'wild':
    ...
    if eval_mode:
        ...
        ################################################
    models, cfgs = load_models(dataset=dataset, is_origin=is_origin_model)
    #################################################################################
    data = prepare_data(name=dataset, root='datasets', eval_mode=eval_mode, subname=name_g)
    print(22222222222222222222222222222222222222)

    # print(data.gallery)
    query_dataloader = create_query_dataloader(
        data.query, cfg=cfgs[1], root=root, eval_mode=eval_mode, name=name_q)
    gallery_dataloader_dict = create_gallery_dataloader(
        data.gallery, cfg=cfgs[1], num_cam=n_cam, root=root, eval_mode=eval_mode, name=name_g)
    ...
    dist = predict(query_features, query_cameras, gallery_features, cfgs[1])

    array_save_path = 'orange_demo/mv_video/0119/dist'
    import numpy as np
    ...
    if is_origin_model:
        file_head = 'origin_'
    with open('mtl/mv_results/' + dataset + '/' + file_head + 'auto_log_cor_w_minp.txt', 'w') as f:
        sys.stdout = f
        print('by distance')
