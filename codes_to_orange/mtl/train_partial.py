from mv_eval_partial import load_models, prepare_data, create_train_dataloader
...
def model_train(num_cam, epochs, model_dict, fcs, train_dataloader, optimizers, save_path, num_classes_dict, device):
    for epoch in range(epochs):
        ...
        for i, datas in enumerate(tqdm(train_dataloader)):
            ...
            feas_total = {}
            for i in range(8):
                feas_total[i] = torch.zeros(2, 256).to(device)
            for i, ft in feas_total.items():
                for j, fea in feas.items():
                    # 需要验证一下fea的格式
                    # 把[1,6]和[2,6]对应成[0,1]
                    ft[int(j) // 6] = fea[i]  # 维度不匹配，update时特征维度是[num_classes]
            feas_out_list = []
...
if __name__ == "__main__":
    # 设置随机数种子
    seed = 24  # 你可以选择任何种子值, 测试的时候设置了111
    ...
    if dataname == 'wild':
        ...
        n_cam = 6
        cfg = cfgs[2]
    filename = 'mtl/logs_partial/' + dataname + '/reranked_seed' + str(seed) + '_epoch' + str(epochs) + '_relu' + '_hl' + str(nums_hl)
    # filename = 'mtl/logs/wild/' + 'reranked_seed' + str(seed) + '_epoch' + str(epochs) + '_hl' + str(nums_hl)
    ...
    for i in range(8):
        fcs[i] = FC(2, nums_hl, 256).to(device)  # 第一个参数为2，只有两个视角
    paras = {}
    ...
    save_path = os.path.join('mtl/saves_partial', dataname, 'reranked_seed' + str(seed) + '_epoch' + str(epochs) + '_relu' + '_hl' + str(nums_hl))
    ...
    torch.cuda.empty_cache()
    model_train(n_cam, epochs, models, fcs, train_dataloader, optimizers, save_path, num_classes_dict, device)
