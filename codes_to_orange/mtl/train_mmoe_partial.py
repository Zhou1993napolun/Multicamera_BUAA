    ...
    for epoch in range(epochs):
        ...
        for i, datas in enumerate(tqdm(train_dataloader)):
            ...
            if not feas:
                ...
                continue
            # cur_cams = len(feas)
            # 构建 key in [0:8], shape为[2, hidden_size]的空字典，对应位置填充feas值
            feas_total = []
            for i in range(2):  # 这里只使用两个视角
                feas_total.append(torch.zeros(8, 256).to(device))
            for i, ft in feas.items():
                feas_total[int(i) // 6] = torch.cat(ft, dim=0)
            # num_cam * [8, 256] -> [8, 256, num_cam]
            stacked_feas = torch.stack(feas_total, dim=-1)
            reshaped_feas = stacked_feas.view(8, -1)  # [8, 2*256]
...
if __name__ == "__main__":
    ...
    if dataname == 'wild':
        ...
        cam_list = [2, 6]
        cfg = cfgs[2]
    filename = 'mtl/logs_partial/' + dataname + '/mmoe_reranked_seed' + \
    ...
    # 后面其实可以把这些视角总数量2之类的都用len(cam_list)替换了
    ...
    save_path = os.path.join('mtl/saves_partial', dataname, 'mmoe_reranked_seed' + str(
