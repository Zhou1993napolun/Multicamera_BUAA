...
class MGN(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer('pixel_mean', torch.Tensor(
            pixel_mean).view(1, -1, 1, 1), False)
        self.register_buffer('pixel_std', torch.Tensor(
            pixel_std).view(1, -1, 1, 1), False)
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            b1_outputs = self.b1_head(b1_feat, targets)  # [64,256]
            b2_outputs = self.b2_head(b2_feat, targets)  # [64,256]
            b21_outputs = self.b21_head(b21_feat, targets)  # [64,256]
            b22_outputs = self.b22_head(b22_feat, targets)  # [64,256]
            b3_outputs = self.b3_head(b3_feat, targets)  # [64,256]
            b31_outputs = self.b31_head(b31_feat, targets)  # [64,256]
            b32_outputs = self.b32_head(b32_feat, targets)  # [64,256]
            b33_outputs = self.b33_head(b33_feat, targets)  # [64,256]
    ...
    def forward_mmoe(self, batched_inputs, targets):
        images = self.preprocess_image(batched_inputs)
        features = self.backbone(images)  # (bs, 2048, 16, 8)
        ...
        b1_outputs = self.b1_head.forward_mmoe(b1_feat, targets)
        b2_outputs = self.b2_head.forward_mmoe(b2_feat, targets)
        b21_outputs = self.b21_head.forward_mmoe(b21_feat, targets)
        b22_outputs = self.b22_head.forward_mmoe(b22_feat, targets)
        b3_outputs = self.b3_head.forward_mmoe(b3_feat, targets)
        b31_outputs = self.b31_head.forward_mmoe(b31_feat, targets)
        b32_outputs = self.b32_head.forward_mmoe(b32_feat, targets)
        b33_outputs = self.b33_head.forward_mmoe(b33_feat, targets)

        return [b1_outputs, b2_outputs, b21_outputs, b22_outputs, b3_outputs, b31_outputs, b32_outputs, b33_outputs], targets

    def inference_mmoe(self, batched_inputs):
        images = self.preprocess_image(batched_inputs)
        features = self.backbone(images)  # (bs, 2048, 16, 8)
        ...
        b1_pool_feat = self.b1_head.forward_mmoe(b1_feat)
        b2_pool_feat = self.b2_head.forward_mmoe(b2_feat)
        b21_pool_feat = self.b21_head.forward_mmoe(b21_feat)
        b22_pool_feat = self.b22_head.forward_mmoe(b22_feat)
        b3_pool_feat = self.b3_head.forward_mmoe(b3_feat)
        b31_pool_feat = self.b31_head.forward_mmoe(b31_feat)
        b32_pool_feat = self.b32_head.forward_mmoe(b32_feat)
        b33_pool_feat = self.b33_head.forward_mmoe(b33_feat)

        return [b1_pool_feat, b2_pool_feat, b3_pool_feat, b21_pool_feat,
                b22_pool_feat, b31_pool_feat, b32_pool_feat, b33_pool_feat]

    def backward_mmoe(self, features, targets):
        b1_feat, b2_feat, b21_feat, b22_feat, b3_feat, b31_feat, b32_feat, b33_feat = features

        b1_outputs = self.b1_head.forward_mmoe_s2(b1_feat, targets)
        b2_outputs = self.b2_head.forward_mmoe_s2(b2_feat, targets)
        b21_outputs = self.b21_head.forward_mmoe_s2(b21_feat, targets)
        b22_outputs = self.b22_head.forward_mmoe_s2(b22_feat, targets)
        b3_outputs = self.b3_head.forward_mmoe_s2(b3_feat, targets)
        b31_outputs = self.b31_head.forward_mmoe_s2(b31_feat, targets)
        b32_outputs = self.b32_head.forward_mmoe_s2(b32_feat, targets)
        b33_outputs = self.b33_head.forward_mmoe_s2(b33_feat, targets)
        
        losses = self.losses_mmoe(b1_outputs,
                                b2_outputs, b21_outputs, b22_outputs,
                                b3_outputs, b31_outputs, b32_outputs, b33_outputs,
                                targets)
        return losses
        ...
        #                            b22_pool_feat, b31_pool_feat, b32_pool_feat, b33_pool_feat], dim=1)
        #     return pred_feat
    # 个人想法是在训练时，对于mgn再写一个forward方法，将其中的每步特征能够单独提出来，并且经过mmoe组合后（同时除以scale），在放入criterion统一更新
    ...
    def preprocess_image(self, batched_inputs):
        ...
        if isinstance(batched_inputs, dict):
            ...
            raise TypeError("batched_inputs must be dict or torch.Tensor, but get {}".format(
                type(batched_inputs)))
    ...
    def losses(self,
        ...
        b33_pool_feat = torch.cat(
            (b31_pool_feat, b32_pool_feat, b33_pool_feat), dim=1)
    ...
    def losses_mmoe(self,
        ...
        # Log prediction accuracy
        log_accuracy_mmoe(pred_class_logits, gt_labels)
