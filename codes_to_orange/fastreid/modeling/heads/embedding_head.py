...
class EmbeddingHead(nn.Module):
    ...
    def forward(self, features, targets=None):
        ...
        """
        # features [bs,2048,24,8]
        pool_feat = self.pool_layer(features)  # [bs,1024,1,1]
        neck_feat = self.bottleneck(pool_feat)  # [bs,256,1,1]
        neck_feat = neck_feat[..., 0, 0]  # resize to [bs,256]
        ...
        if self.cls_layer.__class__.__name__ == 'Linear':
            ...
            logits = F.linear(F.normalize(neck_feat), F.normalize(self.weight))  # [bs,num_classes], ex. [64,751] for Market1501
    ...
    def forward_mmoe(self, features, targets=None):
        """
        See :class:`ReIDHeads.forward`.
        """
        # features [bs,2048,24,8]
    ...
    def forward_mmoe_s2(self, neck_feat, targets):
        # Training
        if self.cls_layer.__class__.__name__ == 'Linear':
            logits = F.linear(neck_feat, self.weight)
        else:
    ...
    def forward_mmoe_with_target(self, features, targets=None):
        """
        See :class:`ReIDHeads.forward`.
        """
        # features [bs,2048,24,8]
