...
def inference_on_dataset(model, data_loader, evaluator, flip_test=False):
    ...
    with inference_context(model), torch.no_grad():
        for idx, inputs in enumerate(data_loader):
            # inputs为字典格式，其中img_paths为一列表，包含图片完整路径信息（例子'datasets/Wildtrack/query/0585_c7_t0660.png'）
            if idx == num_warmup:
                start_time = time.perf_counter()
                total_compute_time = 0

            start_compute_time = time.perf_counter()
            outputs = model(inputs) #[128,2048]
            # Flip test
            if flip_test:
            ...
            # demo部分更改
            ##########################################################################
            with open('orange_demo/video/0134_c6_t0860to0945/img_path.txt', 'a') as f:
                for item in inputs['img_paths']:
                    f.write(item + '\n')
            ##########################################################################
