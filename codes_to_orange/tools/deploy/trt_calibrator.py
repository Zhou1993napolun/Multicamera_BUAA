...
class FeatEntropyCalibrator(trt.IInt8EntropyCalibrator2):
    ...
    def __init__(self, args):
        ...
        self.data_size = self.batch_size * self.channel * \
            self.height * self.width * trt.float32.itemsize
        self.device_input = cuda.mem_alloc(self.data_size)

    def next_batch(self):
        if self.batch_idx < self.max_batch_idx:
            batch_files = self.imgs[self.batch_idx *
            ...
            for i, f in enumerate(batch_files):
                ...
                assert (img.nbytes == self.data_size //
                        self.batch_size), 'not valid img!' + f
                batch_imgs[i] = img
            self.batch_idx += 1
            logger.info(
                "batch:[{}/{}]".format(self.batch_idx, self.max_batch_idx))
