...
def _pool(type, raw, input, x, kernel_size, stride, padding, ceil_mode):
    ...
    layer = caffe_net.Layer_param(name=layer_name, type='Pooling', bottom=[
                                  log.blobs(input)], top=top_blobs)
...
def _view(input, *args):
    ...
    layer = caffe_net.Layer_param(name=layer_name, type='Reshape', bottom=[
                                  log.blobs(input)], top=top_blobs)
