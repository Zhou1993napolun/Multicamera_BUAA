...
class TrtEngine:
    ...
    def __init__(self, trt_file=None, gpu_idx=0, batch_size=1):
        ...
        self._input, self._output, self._bindings, self._stream = self._allocate_buffers(
            self._context)
    ...
    def inference_on_images(self, imgs, new_size=(256, 128)):
        ...
        if valid_bsz < self._batch_size:
            trt_inputs = np.vstack([trt_inputs, np.zeros(
...
if __name__ == "__main__":
    ...
    if args.input:
        ...
        for img_path in tqdm.tqdm(args.input):
            ...
            np.save(os.path.join(args.output, os.path.basename(
                img_path).split('.')[0] + '.npy'), feat)
